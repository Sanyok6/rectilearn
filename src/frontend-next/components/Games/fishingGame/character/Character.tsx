...
export default function CharacterModel(props: CharacterModelProps) {
	...
	useFrame(() => {
		...
		if (rightHandColliderRef.current && leftHandColliderRef.current) {
			...
			leftHandColliderRef.current.setTranslationWrtParent(leftHandRef.current.position);
		}
	});
	...
	return (
		<Suspense fallback={<capsuleGeometry args={[0.3, 0.7]} />}>
            <PerspectiveCamera fov={120} ref={cam} makeDefault>
				<ThirdPerson camera={cam} object={capRef} zooming={zooming} camBack={camBack} />
			</PerspectiveCamera>
        ...
        <mesh castShadow position={[0, 0.2, 0.2]}>
          ...
          <meshStandardMaterial color="mediumpurple" />
			...
			<group ref={group} {...props} dispose={null} userData={{ camExcludeCollision: true }}>
				<mesh geometry={new BoxGeometry(1, 1, 1)} material={new MeshBasicMaterial({ color: 0x00000 })}></mesh>
			</group>
		</Suspense>
	);
}
